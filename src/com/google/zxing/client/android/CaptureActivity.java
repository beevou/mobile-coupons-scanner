...
*/
    ...
    private void checkBeevouCode(String beevouCode)
    {
    try {
		...
		if (json.has("AuthError") == true) {
				...
				{	
					Toast.makeText(CaptureActivity.this, R.string.incorrect_username_password, Toast.LENGTH_LONG).show(); 
					...
					Toast.makeText(CaptureActivity.this, json.getString("AuthError"), Toast.LENGTH_LONG).show();
				}
			} else {
    ...
    public void manualCode(View v)
    {
    	Intent intent = new Intent(this, ManualCode.class);
    	startActivity(intent);
    }
    ...
    private class Asyn_Task extends AsyncTask<String, String, Void> {
        private final ProgressDialog dialog = new ProgressDialog(CaptureActivity.this);
		...
		try {	
			...
			if (json.getString(VALIDATION_CODE) != null) {
				...
				Intent scanActivity = new Intent(CaptureActivity.this, ScanActivity.class);
				Bundle b = new Bundle();
				b.putInt("validationCode", Integer.parseInt(json.getString(VALIDATION_CODE))); 
				if (json.has("error"))
					b.putString("error", json.getString("error"));
